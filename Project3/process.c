...
>>>>>>> a98d6b0914b00a546ea9dc85dcec2b4202210887
	...
	while(message_stats.num_packets_received < num_packets)
	{
		...
		{
			while(message_stats.num_out < message_stats.free_slots)
			{
				int p = get_next_packet(num_packets);
				send_packet(&message_stats.packet_status[p].packet, message_stats.mailbox_id, message_stats.receiver_info.pid); 
